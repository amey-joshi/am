...
notation for vectors and scalars. Instead, we define every symbol.
...
it provides the germ for the the rest.

\section{RNN with a Single Layer}\label{s4}
Recurrent neural networks, abbreviated as RNN, feed a part of their output
to their input. The output if a step $n$ thus depends on both the input
at that step and the output at the previous step. This makes them similar
...
$\nabla y(x_n) \cdot h$ is in $\mathbb{R}^q$. An RNN corresponding to this
equation is
...
A system of first order ODE is a set of equations of the form
\begin{eqnarray*}
...
where
\[
y = \begin{pmatrix}y_1 \\ \vdots \\ y_k\end{pmatrix},\;\text{and}\;
f = \begin{pmatrix}f_1 \\ \vdots \\ f_k\end{pmatrix}
\]
A system of first order ODE is interesting because they are equivalent to
...
nineteenth centuries\cite{lambert1991numerical}. We will describe the most
...
depends on $y(t_n)$ and four other numbers $k_1, \ldots, k_4$. Of these,
$k_1$ depends on $y(t_n)$ and $t_n$, $k_2$ on $y(t_n)$, $t_n$ and $k_1$,
$k_3$ on $y(t_n), t_n$ and $k_2$ and $k_4$ on $y(t_n), t_n$ and $k_3$. 
...
all of them to get the final result. This is a $4$-stage, $1$st order
RNN.

We now define a general RNN\cite{niu2019recurrent}.
...
\item $\alpha_{q, k}, \beta_k$, for $q = 2, \ldots, n$ and $k = 1, \ldots,
n$, $\gamma_2, \ldots, \gamma_{n}, \kappa_2, \ldots, \kappa_{n}$ are 
$p \times p$ matrices;
...
\begin{rem}
$K_1, \ldots, K_q$ are all $p$-dimensional vectors. That is, they 
...
instability. Because of a correspondence between and RNN and an ODE, we 
...
in $[a, b]$.
\end{defn}
A contractive solutions get closer to each other as one gets away from
the initial point.
...
The GRU described in section \ref{s8} has $\alpha_{i, j} = 0$ because there
is only a single hidden layer. The Burrage-Butcher tensor is thus $-\beta_1
\beta_1^T$, which is \emph{not} positive semidefinite unless $\beta_1 = 0$.
Thus the GRU is not a stable algorithm, an observation also corroborated by
elsewhere\cite{kanai2017preventing}.
