...
We will now analyze GRU with attention layer. We assume that the 
architecture of the network is same as shown in figure 4 of the paper
\cite{ran2019lstm} by Xiangdong and others, except that the LTSM is 
replaced with a GRU. There are $n$ such GRU's, each with an output
$h^i_l \in \mathbb{R}^s$. The combined output of all of them is the $s
