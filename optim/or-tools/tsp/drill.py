...
def compute_distances(locations):
    ...
    distances = [[0] * npoints] * npoints
    for i, i_node in enumerate(locations):
        for j, j_node in enumerate(locations):
            if i != j:
                d = int(math.hypot(i_node[0]-j_node[0], i_node[1]-j_node[1]))
                distances[i][j] = d
                distances[j][i] = d

    return distances

def distance_callback(i_index, j_index):
    i = manager.IndexToNode(i_index)
    j = manager.IndexToNode(j_index)

    return distances[i][j]

def build_route(manager, routing, solution):
    ...
    while not routing.IsEnd(curr):
        points.append(manager.IndexToNode(curr))
        prev = curr
        curr = solution.Value(routing.NextVar(curr))
        dist += routing.GetArcCostForVehicle(prev, curr, 0)

    return {'route': points, 'distance': dist}
   

def main():
    data = create_data_model()
    distances = compute_distances(data['locations'])
    ...
    if solution:
        results = build_route(manager, routing, solution)
        points = results['route']
        distance = results['distance']

        print(f'Route distance: {distance}')
        print('->'.join([p for p in points]))
